...
const DistritosProvincia = () => {
    ...
    const getDistritoImage = (distrito) => {
        const distritoImage = getBackendImageUrl(
            distrito?.imagen_fondo ||
            distrito?.imagen_url ||
            distrito?.imagen ||
            ''
        );
        ...
        if (distritoImage) {
            return distritoImage;
        }

        return getProvinciaImage(provincia);
    };
    ...
    const cargarDatos = async () => {
        try {
            ...
            const [provinciaData, distritosData] = await Promise.all([
                getProvinciaById(id),
                getDistritosByProvincia(id, 1, 100)
            ]);
    ...
    const nextDistrito = () => {
        ...
        setCurrentIndex((prev) =>
            prev === distritos.length - 1 ? 0 : prev + 1
        );
    };
    ...
    const prevDistrito = () => {
        ...
        setCurrentIndex((prev) =>
            prev === 0 ? distritos.length - 1 : prev - 1
        );
    };
        ...
        return (
            <div className="distritos-showcase-page">
                ...
                <main className="distritos-state">
                    ...
                    <p>
                        Aún no se han registrado distritos para la provincia de{' '}
                        {provincia?.nombre || 'esta provincia'}.
                    </p>
                ...
                <section className="distritos-showcase-content">
                    <div className="distritos-left">
                        <div className="distritos-top-actions">
                            ...
                            {isAdmin && (
                                <button
                                    ...
                                    onClick={() => navigate(`/gestionar-distritos?provincia_id=${id}`)}
                                >
                                    Gestionar distritos
                                </button>
                            )}
                        ...
                        <div className="distritos-location">
                            <span></span>
                            <p>
                                Distrito de {provincia?.nombre || 'Provincia'}
                            </p>
                        </div>
                        ...
                        <div className="distritos-details">
                            ...
                            <div>
                                <span>Altitud</span>
                                <strong>
                                    {currentDistrito.altitud_msnm
                                        ? `${formatNumber(currentDistrito.altitud_msnm)} msnm`
                                        : 'No registrada'}
                                </strong>
                            </div>
                        </div>
                    ...
                    <div className="distritos-right">
                            ...
                            {getVisibleCards().map((distrito) => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => {
                                            const index = distritos.findIndex(
                                                (item) => item.id === distrito.id
                                            );
                                    ...
                                    >
                                        ...
                                        <div className="distrito-preview-content">
                                            ...
                                            <small>
                                                {distrito.tipo_zona || 'Distrito'}
                                            </small>

                                            <strong>{distrito.nombre}</strong>

                                            <p>
                                                {distrito.total_ciudades || 0} ciudades
                                            </p>
                                        </div>
                                    </button>
                                );
                        ...
                        <div className="distritos-controls">
                            <button type="button" onClick={prevDistrito}>
                                ‹
                            </button>

                            <button type="button" onClick={nextDistrito}>
                                ›
                            </button>
...
export default DistritosProvincia;