...
import { getCiudadesByDepartamento } from '../services/ciudadService';
import '../styles/DistritosProvincia.css';

const CiudadesDepartamento = () => {
    const { id } = useParams();
    ...
    const cargarDatos = async () => {
        try {
            ...
            const [departamentoData, ciudadesData] = await Promise.all([
                getDepartamentoById(id),
                getCiudadesByDepartamento(id, 1, 100)
            ]);

            setDepartamento(departamentoData?.departamento || departamentoData);
            ...
            setError(err.response?.data?.error || 'Error al cargar ciudades del departamento');
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="distritos-showcase-page">
                ...
                <main className="distritos-state">
                    ...
                    <p>
                        Aún no se han registrado ciudades para el departamento de{' '}
                ...
                <section className="distritos-showcase-content">
                    <div className="distritos-left">
                        <div className="distritos-top-actions">
                            ...
                            {isAdmin && (
                                <button
                                    ...
                                    onClick={() => navigate(`/gestionar-ciudades?departamento_id=${id}`)}
                                >
                                    Gestionar ciudades
                                </button>
                            )}
                        ...
                        <div className="distritos-location">
                            <span></span>
                            <p>
                                Ciudad de {departamento?.nombre || 'Departamento'}
                            </p>
                        </div>
...
export default CiudadesDepartamento;