...
import {
    ...
    updateLugarCiudad,
    deleteLugarCiudad
} from '../services/lugarTuristicoCiudadService';
import {
    getComidasByCiudadId,
    createComidaCiudad,
    updateComidaCiudad,
    deleteComidaCiudad
} from '../services/comidaTipicaCiudadService';
...
const GestionContenidoCiudad = () => {
    ...
    const cargarDatos = async () => {
        try {
            ...
            setError(err.response?.data?.error || 'Error al cargar contenido de la ciudad');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalType === 'lugar') {
                ...
                if (modalMode === 'edit') {
                    await updateLugarCiudad(selectedItem.id, payload);
                } else {
                    await createLugarCiudad(payload);
                }
            }
            ...
            if (modalType === 'comida') {
                ...
                if (modalMode === 'edit') {
                    await updateComidaCiudad(selectedItem.id, payload);
                } else {
                    await createComidaCiudad(payload);
                }
            }
    ...
    const confirmDelete = async () => {
        ...
        try {
            if (deleteType === 'lugar') {
                await deleteLugarCiudad(itemToDelete.id);
            }

            if (deleteType === 'comida') {
                await deleteComidaCiudad(itemToDelete.id);
            }
            ...
            <main className="gestion-contenido-dist-main">
                <section className="gestion-contenido-dist-hero">
                    <div>
                        <button
                            ...
                            onClick={() => navigate('/gestionar-ciudades')}
                        >
                            ← Volver a gestionar ciudades
                        </button>

                        <span>Contenido turístico</span>
                        <h1>{ciudad?.nombre || 'Ciudad'}</h1>
                        <p>
                            Administra los lugares turísticos y comidas típicas propias de esta ciudad.
                        </p>
                    </div>
                    ...
                    <section>
                        <div className="gestion-contenido-dist-section-head">
                            <div>
                                ...
                                <p>Registra los lugares turísticos más importantes de la ciudad.</p>
                            </div>
                        ...
                        {lugares.length === 0 ? (
                            <div className="gestion-contenido-dist-empty">
                                ...
                                <p>Agrega el primer lugar turístico de esta ciudad.</p>
                                        ...
                                        <article className="gestion-contenido-dist-card" key={lugar.id}>
                                            ...
                                            <div className="gestion-contenido-dist-card-body">
                                                ...
                                                <div className="gestion-contenido-dist-actions">
                                                    <button type="button" className="edit" onClick={() => openEditLugar(lugar)}>
                                                        Editar
                                                    </button>

                                                    <button type="button" className="delete" onClick={() => openDeleteModal('lugar', lugar)}>
                                                        Eliminar
                                                    </button>
                                                </div>
                                            </div>
                ...
                {activeTab === 'comidas' && (
                    <section>
                        <div className="gestion-contenido-dist-section-head">
                            <div>
                                <span>Gastronomía urbana</span>
                                <h2>Comidas típicas</h2>
                                <p>Registra los platos típicos o expresiones gastronómicas de la ciudad.</p>
                            </div>
                        ...
                        {comidas.length === 0 ? (
                            <div className="gestion-contenido-dist-empty">
                                ...
                                <p>Agrega la primera comida típica de esta ciudad.</p>
                                        ...
                                        <article className="gestion-contenido-dist-card" key={comida.id}>
                                            ...
                                            <div className="gestion-contenido-dist-card-body">
                                                ...
                                                <div className="gestion-contenido-dist-meta">
                                                    ...
                                                    <strong>{comida.origen_descripcion || 'No registrado'}</strong>
                                                </div>

                                                <div className="gestion-contenido-dist-actions">
                                                    <button type="button" className="edit" onClick={() => openEditComida(comida)}>
                                                        Editar
                                                    </button>

                                                    <button type="button" className="delete" onClick={() => openDeleteModal('comida', comida)}>
                                                        Eliminar
                                                    </button>
                                                </div>
                                            </div>
                ...
                <div className="gestion-contenido-dist-modal-overlay">
                    <div className="gestion-contenido-dist-modal">
                        <div className="gestion-contenido-dist-modal-head">
                            <div>
                                <span>{modalType === 'lugar' ? 'Lugar turístico' : 'Comida típica'}</span>
                                <h2>{modalMode === 'edit' ? 'Editar contenido' : 'Nuevo contenido'}</h2>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="gestion-contenido-dist-form">
                            {modalType === 'lugar' && (
                                <>
                                    <label>
                                        Nombre *
                                        <input type="text" name="nombre" value={lugarForm.nombre} onChange={handleLugarChange} required />
                                    </label>

                                    <label>
                                        Ubicación referencial
                                        <input type="text" name="ubicacion_referencial" value={lugarForm.ubicacion_referencial} onChange={handleLugarChange} />
                                    </label>

                                    <label>
                                        Descripción *
                                        <textarea name="descripcion" value={lugarForm.descripcion} onChange={handleLugarChange} rows="5" required></textarea>
                                    </label>
                                </>
                            )}
                            ...
                            {modalType === 'comida' && (
                                <>
                                    <label>
                                        Nombre *
                                        <input type="text" name="nombre" value={comidaForm.nombre} onChange={handleComidaChange} required />
                                    </label>

                                    <label>
                                        Origen / referencia
                                        <input type="text" name="origen_descripcion" value={comidaForm.origen_descripcion} onChange={handleComidaChange} />
                                    </label>

                                    <label>
                                        Descripción *
                                        <textarea name="descripcion" value={comidaForm.descripcion} onChange={handleComidaChange} rows="5" required></textarea>
                                    </label>
                                </>
                            )}
                            ...
                            <label>
                                Imagen
                                <input type="file" accept="image/png,image/jpeg,image/jpg,image/webp" onChange={handleImageChange} />
                            </label>
...
export default GestionContenidoCiudad;